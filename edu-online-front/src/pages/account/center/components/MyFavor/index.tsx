...
import { queryMyFavorArticle } from '../../service';

const { Paragraph } = Typography;

const MyFavorArticle: React.FC = () => {
  ...
  const {loading, loadMore, loadingMore} = useRequest(
    () => {
      const result = queryMyFavorArticle({userId: initialState?.currentUser.id, current: currentPage, pageSize: pageSize});
      return result;
    },
    {
      onSuccess: (result) => {
  ...
  function changePage(_page: number, _pageSize: number) {
    queryMyFavorArticle({userId: initialState?.currentUser.id, current: _page, pageSize: _pageSize})
        .then((result) => {
